...
sudo docker compose exec db mysqldump -u root -p[PASSWORD] masterdata_kpi > ~/backups/masterdata_backup_$(date +%Y%m%d_%H%M%S).sql
```
...
sudo docker compose exec -T db mysql -u root -p[PASSWORD] masterdata_kpi < ~/backups/masterdata_backup_YYYYMMDD_HHMMSS.sql

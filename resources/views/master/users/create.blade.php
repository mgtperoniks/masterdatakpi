...
<div class="max-w-4xl mx-auto">
    <div class="mb-6 flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-slate-500 text-sm">Fill in the fields to create a new system access.</p>
        </div>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-white dark:bg-card-dark rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
            ...
            <div class="space-y-2">
                ...
                <select name="role" required class="w-full bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 focus:ring-primary focus:border-primary">
                    <option value="admin_dept" {{ old('role') == 'admin_dept' ? 'selected' : '' }}>Admin Departemen</option>
                    <option value="manager" {{ old('role') == 'manager' ? 'selected' : '' }}>Manager</option>
                    <option value="direktur" {{ old('role') == 'direktur' ? 'selected' : '' }}>Direktur</option>
                    <option value="mr" {{ old('role') == 'mr' ? 'selected' : '' }}>Management Rep</option>
                </select>
            </div>
            ...
            <div class="md:col-span-2 space-y-3">
                ...
                <div class="flex flex-wrap gap-4 p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="allowed_apps[]" value="masterdata-kpi" class="rounded border-slate-300 text-primary focus:ring-primary" checked>
                        <span class="text-sm font-medium">Master Data Hub</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="allowed_apps[]" value="kpi-bubut" class="rounded border-slate-300 text-primary focus:ring-primary">

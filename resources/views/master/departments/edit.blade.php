...
@section('header_title', 'Update Department Details')
...
@section('content')
    ...
    <div class="max-w-4xl mx-auto px-1 pb-24 lg:pb-8">
        ...
        <header class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-2xl font-bold tracking-tight">Edit Master Department</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">Update organizational details for <span
                        class="font-bold text-slate-900 dark:text-white">{{ $department->code }}</span></p>
            </div>
        ...
        <form method="POST" action="{{ route('master.departments.update', $department) }}" class="space-y-6">
            @csrf
            @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label
                                    ...
                                    Code</label>
                                <input type="text" value="{{ $department->code }}" readonly
                                ...
                                <p class="text-[9px] text-slate-400 mt-1 italic">Codes cannot be modified.</p>
                            </div>
                            <div>
                                <label

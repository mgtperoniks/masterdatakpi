...
@section('title', 'Edit Heat Number')
@section('header_title', 'Update Heat Number Details')
...
@section('content')
    ...
    <div class="max-w-4xl mx-auto px-1 pb-24 lg:pb-8">
        ...
        <header class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-2xl font-bold tracking-tight">Edit Heat Number</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400">Update tracking data for <span
                        class="font-bold text-slate-900 dark:text-white">{{ $heatNumber->heat_number }}</span></p>
            </div>
        ...
        <form method="POST" action="{{ route('master.heat-numbers.update', $heatNumber) }}" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="p-6 lg:p-8 space-y-6">
                    ...
                    <section>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label
                                    class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5 block">Heat
                                    Number</label>
                                <input type="text" name="heat_number"
                                    value="{{ old('heat_number', $heatNumber->heat_number) }}" required
                    ...
                    <section>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label
                                    class="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5 block">Cor
                                    Qty</label>
                                <input type="number" name="cor_qty" value="{{ old('cor_qty', $heatNumber->cor_qty) }}"
                                    required min="0"
                                    class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border-none rounded-2xl text-sm focus:ring-2 focus:ring-primary transition-all shadow-sm outline-none">
                            </div>
                            <div>
                                ...
                                <div class="relative">
                                    <select name="status"
                                        class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-900 border-none rounded-2xl text-sm focus:ring-2 focus:ring-primary appearance-none outline-none shadow-sm transition-all">
                                        <option value="active" @selected($heatNumber->status === 'active')>Active</option>
                                        <option value="inactive" @selected($heatNumber->status === 'inactive')>Inactive
                                        </option>
                                    </select>
                                    <span
                                        class="material-icons absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">expand_more</span>

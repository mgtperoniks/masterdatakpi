...
@section('title', 'Heat Numbers - ' . $heatDate->format('d M Y'))
@section('header_title', 'Heat Numbers Details')
...
@section('content')
    ...
    <header class="mb-6 lg:flex lg:justify-between lg:items-center">
        ...
        <div class="flex gap-2 ml-8 lg:ml-0">
            <a href="{{ route('master.heat-numbers.create') }}"
                class="h-10 px-4 rounded-xl bg-primary text-white shadow-lg shadow-primary/20 flex items-center justify-center gap-2 font-bold active:scale-95 transition-all text-sm">
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-3">
            <div class="md:col-span-2 relative">
                ...
                <input type="text" name="q" value="{{ request('q') }}"
                    ...
                    placeholder="Search heat number, item code, or name...">
            </div>
    ...
    <div class="space-y-3 pb-24">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3 mt-1 flex-wrap">
                            <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-medium">Prod:
                                {{ $hn->kode_produksi ?? '-' }}
                            </p>
                            ...
                            @if($hn->size)
                                <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-medium">Size: <span
                                        class="text-slate-700 dark:text-slate-200 font-bold">{{ $hn->size }}</span>
                                </p>
                            @endif
                            @if($hn->customer)
                                <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-medium">Customer: <span
                                        class="text-blue-600 dark:text-blue-400 font-bold">{{ $hn->customer }}</span>
                                </p>
                            @endif
                            @if($hn->line)
                                <p class="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-medium">Line: <span
                                        class="text-emerald-600 dark:text-emerald-400 font-bold">{{ $hn->line }}</span>
                                </p>
                            @endif
                        </div>
                    </div>
        ...
        @empty
            <div class="py-20 text-center">
                ...
                <p class="text-slate-400">No heat numbers found for this date.</p>
            </div>
        @endforelse

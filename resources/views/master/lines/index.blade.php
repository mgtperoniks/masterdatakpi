...
@section('header_title', 'Master Lines')
...
@section('content')
    ...
    <header class="mb-6 lg:flex lg:justify-between lg:items-center">
        <div class="mb-4 lg:mb-0">
            <h1 class="text-2xl font-bold tracking-tight">Production Lines</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">Manage production lines and department assignments</p>
        </div>

        <div class="flex gap-2">
            <a href="{{ route('master.lines.create') }}"
    ...
    <div class="space-y-3 pb-24">
        @forelse ($lines as $line)
            <div
                ...
                <div class="flex items-center space-x-4">
                    <div
                        class="w-12 h-12 rounded-xl {{ $line->status === 'active' ? 'bg-rose-50 dark:bg-rose-900/30' : 'bg-slate-100 dark:bg-slate-900/50' }} flex items-center justify-center">
                        <span
                            class="material-icons-outlined {{ $line->status === 'active' ? 'text-rose-600' : 'text-slate-400' }}">reorder</span>
                    </div>
                    <div>
                        <div class="flex items-center space-x-2">
                            <h3 class="font-bold text-slate-900 dark:text-white">{{ $line->name }}</h3>
                            <span
                                class="px-2 py-0.5 rounded-full {{ $line->status === 'active' ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600' : 'bg-slate-100 dark:bg-slate-700 text-slate-400' }} text-[10px] font-bold uppercase tracking-tight">
                                {{ $line->status }}
                            </span>
                        </div>
                        <p class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mt-0.5">
                            {{ $line->code }} | {{ $line->department->name ?? $line->department_code }}
                        </p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-1">
                    ...
                    @if ($line->status === 'active')
                        <form action="{{ route('master.lines.deactivate', $line) }}" method="POST" class="inline">
                            @csrf @method('PATCH')
                            <button type="submit"
                    ...
                    @else
                        <form action="{{ route('master.lines.activate', $line) }}" method="POST" class="inline">
                            @csrf @method('PATCH')
                            <button type="submit"
        ...
        @empty
            <div class="py-20 text-center">
                <span class="material-icons text-slate-200 text-6xl mb-4">reorder</span>
                <p class="text-slate-400">No production lines found.</p>
            </div>
        @endforelse

        {{-- Pagination --}}
        <div class="mt-6">
            {{ $lines->onEachSide(1)->links('layouts.partials.pagination_new') }}
        </div>
    </div>
